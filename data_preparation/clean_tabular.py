...
def create_product_data(df_product: pd.DataFrame, df_image: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df_product (pd.DataFrame): Original product dataframe
        df_image (pd.DataFrame): Processed image dataframe (with "image_data" column generated)
    ...
    def get_image_data(product_id: str) -> Optional[Tuple[List[np.ndarray], int]]:
        ...
        """
        df_this_image = df_image[df_image["product_id"] == product_id]

        try:
            result = []

            for index, image_data in df_this_image["image_data"].iteritems():
...
def get_clean_product_data(df_product: pd.DataFrame, df_image: pd.DataFrame, cached_path: str = "./data/") -> pd.DataFrame:
    """
    ...
    clean_product_path = cached_path + "product_clean.pkl"
    ...
    try:
        ...
        df_product_clean = create_product_data(df_product, df_image)
